...
CONs:
  ...
  * diagrams aren't treated as images anymore and you can't use 'save as' by clicking on them.\
  But caption for diagram is reference to a SVG file in this case, so you are still able to save diagrams as files
  ...
  *(probably it could be workaronded but no recipe yet)*
...
Some times you'll want to or need to specify a customizations like *width* or *hight* of diagrams
...
and other things like *post processing*, *multiple images* in a row.

> Probably you won't want to use customizations at all when you in the beginning
> or in hurry to capture your ideas,
> but if you need some - start from here.

Most of customizations are specified via [R chunk](#usage-customize-rChunks) options.

If it's required to download rendered diagram in format other than SVG, use [`dformat`](#usage-format-renderer)
argument for `to_diagram` function.
...
[R Chunks](https://bookdown.org/yihui/rmarkdown/r-code.html) is a way to embed graphics data in R Markdown's.

> Other way is to use markdown syntax like `![<alt text>](<path to image>){<options>}`,
...
More info about otions could be found [here](https://yihui.org/knitr/options/)
...
use option hooks to automatically set derive one options from another. See more about this in followin sections.
...
You can even define hooks for you own options and specify this options for chunks.
...
just by adjusting only those values without need to jump over files.
...
`"``r <R expressions>``"`, replacing `<R expressions>` with your R code.
...
params:
  ...
  fig_ext: "`if (knitr::is_latex_output()) 'png' else NULL`"
```
...
#. Define global options if necessary, use parameters if need to adjust options
...
#. Define all the necessary options templates for your diagrams embedding.
   ...
   our you could define universal tempaltes's values dependng on output formats using
  `if (knitr::is_<html/latex>_otput()) <value1> else <value2>` pattern.
...
Then you'll want for sure to limit hight for large drawings, see [this](#usage-customize-rChunks-height),
...
To add page break just add the following line into document:
...
into doc in `index.Rmd` file. More about this is [TODO: devdoc-swissknife anatomy](#anatomy).

Function renders diagram from textual description and inserts it into document.
